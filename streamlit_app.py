...
with tab_entry:
  st.markdown(
      "##### 🔍 Select Product Specifications (Auto-Fills Details or Add New)"
  )
  ...
  with col_s1:
    ...
    if sel_sz == "➕ Add New...":
      final_size = st.text_input(
          "Type New Size *", key=f"new_sz_{key_suffix_rill}"
      )
    else:
  ...
  with col_s2:
    ...
    if sel_gsm == "➕ Add New...":
      final_gsm = st.text_input(
          "Type New GSM *", key=f"new_gsm_{key_suffix_rill}"
      )
    else:
  ...
  if has_unselected:
    ...
    if not match_df.empty:
      ...
      action_type = st.radio(
          "Transaction Type", ["Purchased (+)", "Used (-)"], index=1, horizontal=None, key=f"tab1_type_{key_suffix}"
      )
...
with tab_entry:
    ...
    if not (final_p and final_w and final_l and final_g):
        ...
        with col_t1:
            action_type = st.radio("Transaction Type", ["Purchased", "Used"], index=1, horizontal=True, key=f"tab2_type_{key_suffix}")
        with col_t2:
