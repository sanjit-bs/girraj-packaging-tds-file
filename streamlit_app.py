...
def calculate_current_stock(live_df, product, width, length, gsm):
    ...
    mask = (
        (live_df["Product"].str.lower() == str(product).strip().lower()) &
        (live_df["Width"].str.lower() == str(width).strip().lower()) &
        (live_df["Length"].str.lower() == str(length).strip().lower()) &
        (live_df["GSM"].str.lower() == str(gsm).strip().lower())
    )
